...
async def _(client, message):
    ...
    if babi:
        _ping = f"""
{pong}pong : {str(delta_ping_formatted).replace('.', ',')} ms
{tion}owner : {client.me.mention}
{yubot}ubot : {bot.me.mention}
    """
        await message.reply(_ping)
    else:
        await message.reply(f"pong : {str(delta_ping_formatted).replace('.', ',')} ms")
