...
__MODULE__ = "ᴏɴɢɴᴇʀ"
__HELP__ = """**「 BANTUAN UNTUK MODULE ONGNER 」**

𖠇➛ **ᴘᴇʀɪɴᴛᴀʜ: .cping - .caddbl - .climit - .calive**
𖠇➛ **ᴘᴇʀɪɴᴛᴀʜ: .natz gantenk ga - .tes on**
𖠇➛ **ᴘᴇʀɪɴᴛᴀʜ: .p - .ok - .sip - .love - .haha - .kuda**"""
...
async def _(client, message):
    try:
        ...
        if babi:
            _ping = f"""
{pong} ᴘᴏɴɢ : {str(delta_ping_formatted).replace('.', ',')} ms
{tion} ᴜsᴇʀʙᴏᴛ ᴏɴ ʙᴀɴɢ ᴠᴇɴᴏ
"""
            await message.reply(_ping)
        else:
            await message.reply(f"ᴘᴏɴɢ : {str(delta_ping_formatted).replace('.', ',')} ms\nᴜsᴇʀʙᴏᴛ ᴏɴ ʙᴀɴɢ ᴠᴇɴᴏ\n\n")
...
async def _(client, message):
    ...
    try:
        ...
        if chat_id in blacklist:
            txt = f"""
<b>{grp} ɢʀᴏᴜᴘ: {message.chat.title}</b>\n<b>{ktrn} ᴋᴇᴛ: sᴜᴅᴀʜ ᴀᴅᴀ ᴅᴀʟᴀᴍ ʟɪsᴛ ᴊᴇᴍʙᴏᴛ</b>
"""
        else:
            await add_to_vars(client.me.id, "BL_ID", chat_id)
            txt = f"""
<b>{grp} ɢʀᴏᴜᴘ: {message.chat.title}</b>\n<b>{ktrn} ᴋᴇᴛ: ʙᴇʀʜᴀsɪʟ ᴅɪ ᴛᴀᴍʙᴀʜᴋᴀɴ ᴋᴇ ᴅᴀʟᴀᴍ ʟɪsᴛ ᴊᴇᴍʙᴏᴛ</b>
"""
    ...
    if status and hasattr(status, "text"):
        ...
        if pjg <= 100:
            if client.me.is_premium:
                text = f"""
<b>{pong} sᴛᴀᴛᴜs ᴀᴋᴜɴ ᴘʀᴇᴍɪᴜᴍ : ᴛʀᴜᴇ</b>
<b>{tion} ʟɪᴍɪᴛ ᴄʜᴇᴄᴋ : ᴀᴋᴜɴ ᴀɴᴅᴀ ᴛɪᴅᴀᴋ ᴅɪʙᴀᴛᴀsɪ</b>
<b>{yubot} ᴜʙᴏᴛ : {bot.me.mention}</b>
"""
            else:
                text = f"""
<b>sᴛᴀᴛᴜs ᴀᴋᴜɴ  : ʙᴇʟɪ ᴘʀᴇᴍ ᴅᴜʟᴜ ʏᴀ</b>
<b>ʟɪᴍɪᴛ ᴄʜᴇᴄᴋ : ᴀᴋᴜɴ ᴀɴᴅᴀ ᴛɪᴅᴀᴋ ᴅɪʙᴀᴛᴀsɪ</b>
<b>ᴜʙᴏᴛ : {bot.me.mention}</b>
"""
...
async def _(client, message):
    ...
    await gcs.delete()
    _gcs = f"""
<b>{bcs}ʙʀᴏᴀᴅᴄᴀsᴛ ғᴏʀᴡᴀʀᴅ ᴅᴏɴᴇ</b>
<b>{brhsl} sᴜᴄᴄᴇs {done} ɢʀᴏᴜᴘ</b>
<b>{ggl} ғᴀɪʟᴇᴅ {failed} ɢʀᴏᴜᴘ</b>"""
...
@PY.INDRI("natz gantenk ga")
async def didingantenkga(client, message):
    await message.reply(
       "<b>ya benar dia sangat gantenk sekali\n\n- dia baik\n- dia manis\n- dia lucu\n- dia imut\n- dia konbrut awsjshsjhsjs\n\nidaman banget lah pokonya natz nih</b>")

@PY.INDRI("tes on")
async def teson(client, message):
    await message.reply(
       "<b>on selalu natz gantenk</b>")
